...
function Export-HTMLActivityLog {
    $now = Get-Date
...
function New-NetworkDrive {
    ...
    $null = Invoke-Expression $cleanupCmd 2>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $errorString = if ($null -ne $result) { $result.ToString() } else { "Unknown error" }
...
function Save-ServerSettings {
    param($UNCPath, $Username, [System.Security.SecureString]$Password, $DriveLetter, $KeyName, $ServerName)
    ...
    Set-ItemProperty -Path $key -Name "Password" -Value (ConvertTo-EncryptedString $PlainPassword) -Force
...
function Remove-ServerSettings {
    param($KeyName)
    $key = Join-Path $script:RegRoot $KeyName
    if (Test-Path $key) { Remove-Item -Path $key -Recurse -Force }
}
function Remove-AllServerSettings {
...
function ConvertTo-RegistryKeyName {
...
function Measure-StorageMetrics {
    ...
    if ($metrics.TotalVMs -gt 0) {
        $metrics.AverageVMSizeGB = [math]::Round($metrics.TotalStorageGB / $metrics.TotalVMs, 2)
                    $metrics.StandardDeviationGB = Get-StandardDeviation -Values $metrics.VmSizesGB
    }
...
"@
                ...
                if ($machineData.ReverseIncrementalBackups.Count -gt 0) {
                    ...
                    $vrbStdDev = if ($machineData.ReverseIncrementalBackups.Count -gt 1) {
                                                        Get-StandardDeviation -Values ($machineData.ReverseIncrementalBackups | Select-Object -ExpandProperty SizeGB)
                    } else { 0 }
...
"@
                ...
                if ($machineData.FullBackups.Count -gt 0) {
                    ...
                    $stdDev = if ($machineData.FullBackups.Count -gt 1) {
                                                        Get-StandardDeviation -Values ($machineData.FullBackups | Select-Object -ExpandProperty SizeGB)
                    } else { 0 }
...
function Start-ReportForMappedDrive {
    param(
        $DriveLetter
    )
...
function Add-NewServerProfile {
    ...
    $suggested = if ($available -contains 'V') { 'V' } else { $available | Sort-Object | Select-Object -First 1 }
        ...
        $key = ConvertTo-RegistryKeyName -UNCPath $UNCPath -Username $Username
    ...
    if (New-NetworkDrive -DriveLetter $DriveLetter -UNCPath $UNCPath -Username $Username -Password $SecurePassword) {
            ...
            Save-ServerSettings -UNCPath $UNCPath -Username $Username -Password $SecurePassword -DriveLetter $DriveLetter -KeyName $key -ServerName $ServerName
        Write-Log "✅ Running backup analysis and report generation." 'INFORMATIONAL'
        Start-ReportForMappedDrive -DriveLetter $DriveLetter
        Update-HTMLLog
        Remove-OldLogs -N 3
...
while ($true) {
    ...
    if ($servers.Count -gt 0) {
        ...
        if ($choice -match '^[0-9]+$' -and [int]$choice -ge 1 -and [int]$choice -le $servers.Count) {
            ...
            $settings = Get-ServerSettings -KeyName $selected.Key
            ...
            if ($delChoice -eq 'ALL') {
                ...
                Remove-ServerSettings -KeyName $delKey
